...
def test_progressive_curriculum_application():
    try:
        ...
        mock_base_data = [{'question': 'Test question 1', 'steps': ['Step 1', 'Step 2', 'Step 3'], 'answer': 'Test answer 1'}, {'question': 'Test question 2', 'steps': ['Step A', 'Step B'], 'answer': 'Test answer 2'}]
        mock_dataset.data = mock_base_data.copy()
        trainer = CoCoTrainer(model=Mock(), args=args, train_dataset=mock_dataset, eval_dataset=Mock(), data_collator=Mock())
        print('✓ Trainer and dataset setup completed')
        trainer._last_stage = 0
        trainer._update_for_stage(1)
        mock_dataset.apply_progressive_curriculum.assert_called_once_with(scheduled_stage=1, c_thought=args.c_thought, max_latent_stage=args.max_latent_stage, uniform_prob=args.uniform_prob, pad_latent_to_max=args.pad_latent_to_max, no_cot=False)
        ...
        mock_dataset.apply_progressive_curriculum.assert_called_once_with(scheduled_stage=2, c_thought=args.c_thought, max_latent_stage=args.max_latent_stage, uniform_prob=args.uniform_prob, pad_latent_to_max=args.pad_latent_to_max, no_cot=False)
...
def test_dataloader_refresh():
    try:
        ...
        args.gradient_accumulation_steps = 1
        args.report_to = []
        trainer = CoCoTrainer(model=Mock(), args=args, train_dataset=Mock(), eval_dataset=Mock(), data_collator=Mock())
        ...
        with patch('gc.collect'), patch('torch.cuda.empty_cache'):
            ...
            expected_curriculum_calls = 2
            assert trainer.train_dataset.apply_progressive_curriculum.call_count == expected_curriculum_calls, f'Expected {expected_curriculum_calls} curriculum calls, got {trainer.train_dataset.apply_progressive_curriculum.call_count}'
            expected_dataloader_calls = 2
            assert dataloader_call_count == expected_dataloader_calls, f'Expected {expected_dataloader_calls} dataloader refreshes, got {dataloader_call_count}'
...
def test_curriculum_content_changes():
    try:
        ...
        base_dataset = [{'question': 'What is 2 + 2?', 'steps': ['Add 2 and 2', 'The result is 4'], 'answer': '4'}, {'question': 'What is the capital of France?', 'steps': ['Think about France', 'The capital is Paris'], 'answer': 'Paris'}]
        stage_0_data = create_progressive_latent_dataset(scheduled_stage=0, base_dataset=base_dataset, c_thought=4, max_latent_stage=2, uniform_prob=0.0, pad_latent_to_max=False, no_cot=False)
        stage_1_data = create_progressive_latent_dataset(scheduled_stage=1, base_dataset=base_dataset, c_thought=4, max_latent_stage=2, uniform_prob=0.0, pad_latent_to_max=False, no_cot=False)
        stage_2_data = create_progressive_latent_dataset(scheduled_stage=2, base_dataset=base_dataset, c_thought=4, max_latent_stage=2, uniform_prob=0.0, pad_latent_to_max=False, no_cot=False)
