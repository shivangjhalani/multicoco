            ...
            with patch.object(MultiCoCoRunner, '_setup_logging'):
                with patch.object(MultiCoCoRunner, '_setup_wandb'):
                    ...
                    def mock_create_trainer():
                        ...
                        args.report_to = []
                        trainer = CoCoTrainer(model=runner.model, args=args, train_dataset=runner.train_dataset, eval_dataset=runner.eval_dataset, data_collator=Mock())
...
def test_stage_transitions():
    try:
        ...
        trainer.train_dataset.apply_progressive_curriculum.assert_called_once_with(scheduled_stage=1, c_thought=args.c_thought, max_latent_stage=args.max_latent_stage, uniform_prob=args.uniform_prob, pad_latent_to_max=args.pad_latent_to_max, no_cot=False)
