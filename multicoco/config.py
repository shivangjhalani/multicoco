...
class DataConfig:
    ...
    test_limit: Optional[int] = None  # For development/testing
    ...
    def __post_init__(self):
        ...
        if self.val_data_path:
            self.val_data_path = os.path.abspath(self.val_data_path)
...
class MultiCoCoConfig:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'MultiCoCoConfig':
        ...
        logging_config = LoggingConfig(
            ...
            enable_wandb=not config_dict.get('eval_only', False),  # Disable wandb for eval_only
        )
    ...
    def validate(self) -> None:
        ...
        if not self.evaluation.eval_only and not self.data.train_data_path:
            raise ValueError("Training data path is required when not in eval_only mode")
        
        if self.evaluation.eval_only and not self.data.val_data_path:
            raise ValueError("Validation data path is required in eval_only mode")
    ...
    def get_wandb_report_to(self) -> List[str]:
        ...
        return ["wandb"] if self.logging.enable_wandb else [] 